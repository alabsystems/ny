...
"""
Explicit VNN-COMP benchmark diagnostic for ny.
...
Run:
    pytest vnncomp_diagnostic.py -v --ny-benchmark-timeout=10
    pytest vnncomp_diagnostic.py -v --ny-benchmark-timeout=60 \
        --ny-benchmark-method=beta
    pytest vnncomp_diagnostic.py -v -k acasxu \
        --ny-benchmark-results=results.json
"""
...
def _require_completed_suite(results, label):
    """Require a non-empty suite with no failed or timed-out ny invocation."""
    assert results["total"] > 0, f"{label} contained no runnable instances"
    if results["error"]:
        pytest.fail(f"{label} had {results['error']} ny invocation error(s)")
    if results["timeout"]:
        pytest.fail(
            f"{label} had {results['timeout']} configured-timeout outcome(s)"
        )
    outcome_total = sum(
        results[key] for key in ["verified", "falsified", "unknown", "timeout", "error"]
    )
    assert outcome_total == results["total"], (
        f"{label} outcome counts do not match total: "
        f"{outcome_total} != {results['total']}"
    )
    assert math.isfinite(results["total_time"]) and results["total_time"] >= 0
    assert math.isfinite(results["avg_time"]) and results["avg_time"] >= 0
    assert math.isfinite(results["verified_rate"])
...
class TestVnncompInfrastructure:
    ...
    def test_vnncomp_2023_exists(self):
        ...
        require_benchmark_path(acasxu, "Missing ACAS-Xu 2023")
...
class TestAcasXu2021:
    ...
    def test_full_benchmark(self, verify_timeout, method, request):
        ...
        _require_completed_suite(results, "ACAS-Xu 2021")
        assert results["verified_rate"] > 95, (
            f"Verified rate {results['verified_rate']:.1f}% below target 95%"
        )
        assert results["avg_time"] < 10, (
            f"Average time {results['avg_time']:.2f}s above target 10s"
        )
...
class TestAcasXu2023:
    ...
    def test_full_benchmark(self, verify_timeout, method, request):
        ...
        if save_path:
            with open(save_path, "w") as f:
                json.dump(results, f, indent=2)
        _require_completed_suite(results, "ACAS-Xu 2023")
...
class TestMnist2021:
    ...
    def test_full_benchmark(self, verify_timeout, method, request):
        ...
        _require_completed_suite(results, "MNIST-FC 2021")
    ...
    def test_full_benchmark(self, verify_timeout, method, request):
        ...
        if save_path:
            with open(save_path, "w") as f:
                json.dump(results, f, indent=2)
            print(f"Results saved to: {save_path}")
        _require_completed_suite(results, "Traffic Signs 2023")
...
class TestVnncompAggregate:
    ...
    def test_all_acasxu(self, verify_timeout, method, request):
        ...
        for year in [2021, 2023, 2024, 2025]:
            ...
            _require_completed_suite(results, f"ACAS-Xu {year}")
        ...
        assert total_instances > 0, "Aggregate ACAS-Xu suite contained no instances"
    ...
    def test_benchmark_matrix(self, verify_timeout, method, request):
        ...
        for year, benchmarks in BENCHMARKS_BY_YEAR.items():
            for bench in benchmarks[:3]:  # Limit to first 3 per year for speed
                ...
                _require_completed_suite(results, f"{bench} {year}")
                suites_run += 1
                print(
                    f"{year:<6} {bench:<25} "
        ...
        print(f"{'=' * 80}")
        assert suites_run > 0, "Benchmark matrix contained no runnable suites"
