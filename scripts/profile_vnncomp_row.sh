...
TMPOUT="/tmp/profile_vnncomp_row_${PROFILE_RUN_ID}.stdout"
EARLY_SAMPLE_PATH="/tmp/profile_vnncomp_row_${PROFILE_RUN_ID}_early.sample.txt"
LATE_SAMPLE_PATH="/tmp/profile_vnncomp_row_${PROFILE_RUN_ID}_late.sample.txt"
...
capture_sample_output() {
    ...
    sample_timeout=$(python3 -c "print(max(float('$SAMPLE_DURATION') + 1.0, 2.0))")
    ...
    while process_is_live "$sample_pid"; do
        if ! python3 -c "exit(0 if float('$elapsed') < float('$sample_timeout') else 1)"; then
        ...
        sleep 0.1
        elapsed=$(python3 -c "print(f'{float(\"$elapsed\") + 0.1:.1f}')")
    done
