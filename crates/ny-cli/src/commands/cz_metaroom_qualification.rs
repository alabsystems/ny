...
use ny_mip::{
    ...
    ReluTailBoxCutSelection, ReluTailDualConfig, ReluTailDualLimits, ReluTailDualResult,
    TailLpMarginOutcome,
};
#[cfg(feature = "cuda")]
...
#[cfg(test)]
use ny_onnx::vnnlib::parse_vnnlib_with_certified_input_box;
use ny_onnx::vnnlib::{load_vnnlib_with_certified_input_box, OutputConstraint, VnnLibSpec};
#[cfg(test)]
use ny_onnx::{
    load_onnx_bytes_with_config, Network, ShapeInferencePolicy, TensorSpec, WeightStore,
};
...
fn synthetic_constant_folded_and_fused_parameters_fail_provenance() {
    ...
    let error =
        propagate_direct_onnx_conv2d_unwired(&synthetic, &synthetic_graph, &input_box(), limits())
            .unwrap_err();
    assert!(
        error
            .to_string()
    ...
    let error = propagate_direct_onnx_conv2d_unwired(&fused, &fused_graph, &input_box(), limits())
        .unwrap_err();
    assert!(
        error
            .to_string()
...
fn post_load_weight_mutation_fails_even_when_graph_matches() {
    ...
    let graph = model.to_graph_network().unwrap();
    let error =
        propagate_direct_onnx_conv2d_unwired(&model, &graph, &input_box(), limits()).unwrap_err();
    assert!(
        error
            .to_string()
            .contains("no longer matches its private raw ONNX FLOAT initializer provenance"),
...
fn malformed_raw_rank_and_oversized_topology_are_bounded() {
    ...
    let error =
        propagate_direct_onnx_conv2d_unwired(&oversized_topology, &graph, &input_box(), limits())
            .unwrap_err();
    assert!(
        matches!(
            &error,
    ...
    oversized_capacity.network.layers[0]
        ...
        .reserve(4_097);
    let error =
        propagate_direct_onnx_conv2d_unwired(&oversized_capacity, &graph, &input_box(), limits())
            .unwrap_err();
    assert!(
        matches!(
            &error,
...
fn relu_topology_and_resource_drift_fail_closed() {
    ...
    propagate_direct_onnx_conv2d_relu_unwired(&model, &graph, &input_box(), relu_limits()).unwrap();
}
...
fn every_preallocation_cap_fails_before_expansion() {
    ...
    let error = propagate_direct_onnx_conv2d_unwired(&model, &graph, &box_, capped).unwrap_err();
...
fn real_metaroom_119_builds_161_symbol_first_conv_stem() {
    ...
    assert!(
        onnx.is_file() && vnnlib.is_file(),
        "external metaroom fixtures missing (model={}, property={}); run \
...
impl MetaroomBoxDiagnostic {
    ...
    fn record_relu(
        ...
        let (cz_bounds, hull_plan) =
            unconstrained_zonotope_box_unwired(cz_preactivation, metaroom_box_limits()).unwrap();
...
pub(crate) fn real_metaroom_119_measures_full_conv_relu_trunk_resources() {
    qualify_real_metaroom_119_full_conv_relu_trunk(FullTrunkPredicateMode::Preserve)
        .expect("explicit qualification requires the checked Metaroom119 trunk");
}

pub(crate) fn real_metaroom_119_measures_projected_full_conv_relu_trunk_resources() {
    qualify_real_metaroom_119_full_conv_relu_trunk(FullTrunkPredicateMode::ProjectedReluGeometry)
        .expect("explicit projected qualification requires the checked Metaroom119 trunk");
}
...
fn qualify_real_metaroom_119_full_conv_relu_trunk_with_auxiliary_trace(
    ...
    qualify_real_metaroom_119_full_conv_relu_trunk_impl(mode, None, Some(auxiliary_trace), false)
}
...
fn qualify_real_metaroom_119_full_conv_relu_trunk_impl(
    ...
    if box_diagnostic.is_some() || is_hybrid {
        ...
        let (conv1_output, conv1_plan) =
            propagate_direct_onnx_conv2d_unwired(&model, &graph, &box_, first_limits.stem).unwrap();
...
fn seal_and_promote_linear(
    ...
    let graph_bias = graph_linear.bias().unwrap();
...
pub(crate) fn real_metaroom_119_seals_affine_tail_topology_and_float32_bits() {
    let root = std::env::var_os("NY_METAROOM_119_ROOT")
        .map(PathBuf::from)
        .unwrap_or_else(|| {
            Path::new(env!("CARGO_MANIFEST_DIR"))
    ...
    assert!(
        onnx.is_file(),
        "explicit qualification requires the Metaroom119 model at {}",
        onnx.display()
    );
    let config = OnnxLoadConfig::default().with_raw_float32_initializer_provenance(true);
    let model = load_onnx_with_config(&onnx, &config).unwrap();
...
pub(crate) fn real_metaroom_119_measures_full_affine_relu_output_tail_resources() {
    let qualified =
        qualify_real_metaroom_119_full_affine_relu_output_tail(FullTrunkPredicateMode::Preserve)
            .expect("explicit qualification requires the checked Metaroom119 assets");
    assert_eq!(qualified.domain.value_dim(), 20);
    assert_eq!(qualified.vnnlib_spec.num_outputs, 20);
}
...
fn qualify_real_metaroom_119_full_affine_relu_output_tail(
    ...
    let first_output_generator_ceiling = trunk.relu4_output.alpha_dim().checked_mul(256).unwrap();
...
#[cfg(feature = "cuda")]
pub(crate) fn real_metaroom_119_measures_projected_full_output_cuda_dual() {
...
pub(crate) fn real_metaroom_119_quantifies_certified_box_advantage_over_projected_cz_radii() {
...
pub(crate) fn real_metaroom_119_measures_inductive_box_cz_hybrid_resources() {
    let mut diagnostic = MetaroomBoxDiagnostic::default();
    ...
    for (baseline_row, hybrid_row) in baseline_resources.iter().zip(hybrid.relu_resources.iter()) {
    ...
    assert!(hybrid.relu_resources[3].generator_nonzeros < baseline_resources[3].generator_nonzeros);
    assert!(hybrid.relu_resources[3].constraint_count < baseline_resources[3].constraint_count);
    assert!(
        hybrid.relu_resources[3].constraint_elements < baseline_resources[3].constraint_elements
    );
    ...
    assert!(hybrid_affine.output_generator_nonzeros < baseline_affine.output_generator_nonzeros);
...
const METAROOM_TAIL_POSITIVE_COEFFICIENTS: [usize; 20] = [
    116, 112, 109, 100, 110, 107, 0, 124, 111, 115, 120, 122, 95, 123, 135, 102, 107, 116, 109, 109,
];
...
impl<'domain> MetaroomPreparedTailDomain<'domain> {
    fn try_new(domain: &'domain ConstrainedZonotope64) -> Result<Self, ny_mip::ReluTailDualError> {
    ...
    for &challenger_output in challengers {
        ...
        let result = prepared
            ...
            .bound_margin_with_optimized_auxiliary_box_cut_unwired(
                ...
                &margin,
                None,
                metaroom_tail_dual_config(iterations, remaining.min(METAROOM_M24_MEMBER_WALL_CAP)),
...
fn hybrid_tail_portfolio_tie_and_auxiliary_error_preserve_m17() {
    let (tie, error) = retain_metaroom_tail_portfolio::<&'static str>(Ok(0.0), Ok(0.0)).unwrap();
    assert_eq!(error, None);
...
fn m24_portfolio_failures_retain_best_prior_certificate() {
    ...
    let (both_optional_fail, errors) =
        retain_metaroom_m24_portfolio(Ok(-2.0), Err("M20 failed"), Err("M24 unavailable")).unwrap();
...
fn hybrid_tail_prepared_geometry_serves_both_portfolio_members() {
    ...
    let auxiliary = CertifiedAuxiliaryBounds64::try_new(vec![-1.0, 0.0], vec![1.0, 2.0]).unwrap();
...
pub(crate) fn real_metaroom_119_diagnoses_inductive_hybrid_tail_cascade() {
...
pub(crate) fn real_metaroom_119_diagnoses_target_6_tail_with_exact_ay_lp() {
