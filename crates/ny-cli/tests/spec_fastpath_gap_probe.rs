...
//! `cargo test -p ny-cli --release --test spec_fastpath_gap_probe -- --ignored --nocapture`.
...
fn run_probe(label: &str, onnx_rel: &str, vnnlib_rel: &str) {
    let onnx_path = Path::new(BENCH).join(onnx_rel);
    let vnnlib_path = Path::new(BENCH).join(vnnlib_rel);
    let model = load_onnx_with_config(&onnx_path, &OnnxLoadConfig::default()).expect("load onnx");
    ...
    let graph = model
        ...
        .expect("graph");
    let vnnlib = load_vnnlib(&vnnlib_path).expect("vnnlib");
