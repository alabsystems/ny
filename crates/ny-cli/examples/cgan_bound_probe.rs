...
//! This is an explicit measurement lane, not a test. Run it with
//! `cargo run -p ny-cli --release --example cgan_bound_probe -- <name>`.
...
fn require_file(path: &str, probe: &str) -> anyhow::Result<()> {
    anyhow::ensure!(
        Path::new(path).is_file(),
        "{probe}: missing ONNX prerequisite {path}"
    );
    Ok(())
}
...
fn run_named(name: &str) -> anyhow::Result<()> {
    match name {
        "bound" => {
            require_file(ONNX, name)?;
            cgan_bound_probe();
        }
        "oracle-nch1" => {
            require_file(ONNX, name)?;
            cgan_oracle_parity_nch1();
        }
        "oracle-nch3" => {
            require_file(ONNX_NCH3, name)?;
            cgan_oracle_parity_nch3();
        }
        "bn5-exact" => {
            require_file(ONNX, name)?;
            cgan_bn5_pure_affine_exactness();
        }
        "sampling" => {
            require_file(ONNX, name)?;
            cgan_sampling_soundness_nch1();
        }
        "precheck" => {
            require_file(ONNX, name)?;
            cgan_per_output_precheck_probe();
        }
        "minimal" => cgan_minimal_affine_chain_probe(),
        "bn-bisect" => cgan_affine_chain_bn_bisect(),
        "manual-steps" => cgan_affine_chain_manual_steps(),
        "alpha-unstable" => {
            require_file(ONNX, name)?;
            cgan_alpha_unstable_probe();
        }
        "alpha-warmup" => {
            require_file(ONNX, name)?;
            cgan_alpha_warmup_replica_probe();
        }
        "all" => {
            for probe in [
                "bound",
        ...
        _ => anyhow::bail!(
            "unknown probe {name:?}; expected bound, oracle-nch1, oracle-nch3, \
             bn5-exact, sampling, precheck, minimal, bn-bisect, manual-steps, \
             alpha-unstable, alpha-warmup, or all"
        ),
    }
    Ok(())
}
...
fn main() -> anyhow::Result<()> {
    ...
    let name = args
        .next()
        .ok_or_else(|| anyhow::anyhow!("missing cGAN probe name; pass a named probe or all"))?;
