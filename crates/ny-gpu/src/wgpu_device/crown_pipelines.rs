...
//! CROWN backward pass GPU pipeline creation (#3397).
//!
//! Split from pipelines.rs for file size compliance. Contains pipeline and
...
impl WgpuDevice {
    pub(super) fn create_crown_activation_backward_pipeline(
        ...
        // buffer aliasing where all 4 sub-arrays read from offset 0.
        let bind_group_layout = device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
            label: Some("crown_activation_backward_bind_group_layout"),
            entries: &[
                // 0: Params uniform
                ...
                // 3: slopes — packed [lower_slope | upper_slope | lower_intercept | upper_intercept]
                wgpu::BindGroupLayoutEntry {
                    binding: 3,
                    visibility: wgpu::ShaderStages::COMPUTE,
                    ty: wgpu::BindingType::Buffer {
        ...
        let pipeline_layout = device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
            label: Some("crown_activation_backward_pipeline_layout"),
            bind_group_layouts: &[Some(&bind_group_layout)],
            immediate_size: 0,
        });

        let pipeline = device.create_compute_pipeline(&wgpu::ComputePipelineDescriptor {
            label: Some("crown_activation_backward_pipeline"),
    ...
    /// Create the ReLU dual-alpha activation backward pipeline (#4313).
    ///
    /// Reuses the same bind-group layout as the standard activation backward
    /// pipeline (same 8 bindings, same `CrownActivationParams`), but dispatches
    /// a different shader with sign-specific coefficient routing for exact
    /// dual-alpha parity.
    pub(super) fn create_crown_activation_relu_dual_alpha_pipeline(
        ...
        device.create_compute_pipeline(&wgpu::ComputePipelineDescriptor {
            label: Some("crown_activation_relu_dual_alpha_pipeline"),
            layout: Some(&pipeline_layout),
            module: &shader,
            entry_point: Some("main"),
            compilation_options: wgpu::PipelineCompilationOptions::default(),
    ...
    pub(super) fn create_crown_bias_accumulate_pipeline(
        ...
        let pipeline_layout = device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
            label: Some("crown_bias_accumulate_pipeline_layout"),
            bind_group_layouts: &[Some(&bind_group_layout)],
            immediate_size: 0,
        });

        let pipeline = device.create_compute_pipeline(&wgpu::ComputePipelineDescriptor {
            label: Some("crown_bias_accumulate_pipeline"),
            layout: Some(&pipeline_layout),
            module: &shader,
            entry_point: Some("main"),
            compilation_options: wgpu::PipelineCompilationOptions::default(),
    ...
    pub(super) fn create_crown_maxpool2d_backward_pipeline(
        ...
        let pipeline_layout = device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
            label: Some("crown_maxpool2d_backward_pipeline_layout"),
            bind_group_layouts: &[Some(&bind_group_layout)],
            immediate_size: 0,
        });

        let pipeline = device.create_compute_pipeline(&wgpu::ComputePipelineDescriptor {
            label: Some("crown_maxpool2d_backward_pipeline"),
            layout: Some(&pipeline_layout),
            module: &shader,
            entry_point: Some("main"),
            compilation_options: wgpu::PipelineCompilationOptions::default(),
