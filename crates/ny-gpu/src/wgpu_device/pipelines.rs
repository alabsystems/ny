...
use super::shaders::{
    ...
    SCALE_IBP_SHADER, SOFTMAX_APPLY_SHADER, SOFTMAX_REDUCE_SHADER, TRANSPOSE_IBP_SHADER,
};
...
impl WgpuDevice {
    ...
    /// Create the small-K GEMM pipeline, sharing the bind group layout from
    /// the tiled GEMM pipeline. Both shaders use the same bindings (params, A,
    /// B, out), so pre-built bind groups work with either pipeline.
    ///
    ...
    pub(super) fn create_gemm_f32_small_k_pipeline(
        ...
        device.create_compute_pipeline(&wgpu::ComputePipelineDescriptor {
            label: Some("gemm_f32_small_k_pipeline"),
            layout: Some(&pipeline_layout),
            module: &shader,
            entry_point: Some("main"),
            compilation_options: wgpu::PipelineCompilationOptions::default(),
    ...
    /// Create resident Conv2d IBP pipeline: fused patch gather + interval
    ...
    pub(super) fn create_conv2d_ibp_pipeline(
        ...
        let pipeline_layout = device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
            label: Some("conv2d_ibp_pipeline_layout"),
            bind_group_layouts: &[Some(&bind_group_layout)],
            immediate_size: 0,
        });

        let pipeline = device.create_compute_pipeline(&wgpu::ComputePipelineDescriptor {
            label: Some("conv2d_ibp_pipeline"),
            layout: Some(&pipeline_layout),
            module: &shader,
            entry_point: Some("main"),
            compilation_options: wgpu::PipelineCompilationOptions::default(),
    ...
    /// Create ReLU IBP pipeline: in-place max(x, 0) on lower/upper buffers.
    ...
    pub(super) fn create_relu_ibp_pipeline(
        ...
        let bind_group_layout = device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
            ...
            entries: &[
                ...
                // Lower bounds (read_write, in-place)
                wgpu::BindGroupLayoutEntry {
                    binding: 1,
                    visibility: wgpu::ShaderStages::COMPUTE,
                    ty: wgpu::BindingType::Buffer {
                ...
                // Upper bounds (read_write, in-place)
                wgpu::BindGroupLayoutEntry {
                    binding: 2,
                    visibility: wgpu::ShaderStages::COMPUTE,
                    ty: wgpu::BindingType::Buffer {
        ...
        let pipeline_layout = device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
            label: Some("relu_ibp_pipeline_layout"),
            bind_group_layouts: &[Some(&bind_group_layout)],
            immediate_size: 0,
        });

        let pipeline = device.create_compute_pipeline(&wgpu::ComputePipelineDescriptor {
            label: Some("relu_ibp_pipeline"),
            layout: Some(&pipeline_layout),
            module: &shader,
            entry_point: Some("main"),
            compilation_options: wgpu::PipelineCompilationOptions::default(),
    ...
    /// Create Add IBP pipeline: element-wise interval addition for residual
    ...
    pub(super) fn create_add_ibp_pipeline(
        ...
        let pipeline_layout = device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
            label: Some("add_ibp_pipeline_layout"),
            bind_group_layouts: &[Some(&bind_group_layout)],
            immediate_size: 0,
        });

        let pipeline = device.create_compute_pipeline(&wgpu::ComputePipelineDescriptor {
            label: Some("add_ibp_pipeline"),
            layout: Some(&pipeline_layout),
            module: &shader,
            entry_point: Some("main"),
            compilation_options: wgpu::PipelineCompilationOptions::default(),
    ...
    /// Create AvgPool IBP pipeline: windowed/global average pooling for
    ...
    pub(super) fn create_avgpool_ibp_pipeline(
        ...
        let pipeline_layout = device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
            label: Some("avgpool_ibp_pipeline_layout"),
            bind_group_layouts: &[Some(&bind_group_layout)],
            immediate_size: 0,
        });

        let pipeline = device.create_compute_pipeline(&wgpu::ComputePipelineDescriptor {
            label: Some("avgpool_ibp_pipeline"),
            layout: Some(&pipeline_layout),
            module: &shader,
            entry_point: Some("main"),
            compilation_options: wgpu::PipelineCompilationOptions::default(),
