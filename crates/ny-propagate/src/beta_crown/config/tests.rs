...
/// default. Only the cifar100 presets opt in (`use_kfsb_multi_branching: true`);
/// every other lane keeps the default so it stays byte-identical to the
...
fn use_kfsb_multi_branching_defaults_to_false() {
    ...
    assert!(
        ...
        "#kfsb-multi must default OFF so non-cifar100 lanes stay byte-identical"
    );
}
...
fn domain_priority_for_mode_respects_verification_direction() {
    ...
    assert_eq!(
        BetaCrownConfig::domain_priority_for_mode(false, -1.0, -5.0).unwrap(),
        1.0
    );
}
