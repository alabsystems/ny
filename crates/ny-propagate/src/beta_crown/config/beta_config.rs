...
pub struct BetaCrownConfig {
    ...
    /// per-node time budget (#4413, #cgan-bn11-budget). A node whose share
    ...
    /// time budget (#4413, #cgan-bn11-budget). `None` (default) keeps the
    /// built-in 12.0 s constant. Presets whose dominant node needs a long
    /// chunked backward (cgan_2023 BN_11, 28,800 dims: ~143 s full collection
    /// measured) raise this so the in-pipeline collection can actually run.
    /// Purely a time-vs-tightness policy knob: any value is sound.
    #[serde(default)]
    ...
    /// Default: 3 (matches alpha-beta-CROWN `topk_objective`)
    ...
    /// cifar100 presets only so every other lane (relational, acasxu,
    ...
    /// Reference: alpha-beta-CROWN `max_split_depth`.
...
impl Default for BetaCrownConfig {
    fn default() -> Self {
        Self {
            ...
            use_kfsb_multi_branching: false, // #kfsb-multi: cifar100-scoped preset opt-in (env NY_MO_KFSB overrides)
...
impl BetaCrownConfig {
    ...
    /// (#4413, #cgan-bn11-budget). All-`None` (the default) keeps the built-in
    /// 2 s floor / 12 s cap constants.
    #[inline]
