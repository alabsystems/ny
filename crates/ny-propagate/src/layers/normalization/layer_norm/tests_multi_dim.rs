...
//! Multi-dimensional scalar LayerNorm CROWN regressions.
//!
//! Part of #4148.
...
/// Trust-verifier regression: LayerNorm IBP used a fixed `[usize; 8]` per-slice
/// index buffer (standard/forward_mode/mean_only/slices), so a tensor of rank > 8
/// indexed it out of bounds and PANICKED. The verifier flagged this
/// (index_out_of_bounds in the LayerNorm IBP path); the fix fails closed.
#[test]
fn layernorm_ibp_rank_over_8_fails_closed_not_panics() {
    ...
    .unwrap();
    assert!(
        layer.propagate_ibp(&pre_act).is_err(),
        "LayerNorm IBP must fail closed (not panic) on rank-9 input"
    );
    // Rank 8 (supported max) must still work.
    ...
    assert!(
        ...
        "LayerNorm IBP must still accept rank-8 input"
    );
}

fn standard_fixture_4148() -> Result<(LayerNormLayer, LinearBounds, BoundedTensor, usize, usize)> {
    let layer = LayerNormLayer::new(arr1(&[1.25, -0.75, 0.5]), arr1(&[0.1, -0.2, 0.3]), 1e-5)?
    ...
    let bounds = LinearBounds::new(
        ...
        arr1(&[0.25, -0.1]),
        arr2(&[
            [1.2, -0.25, 0.4, 0.9, -0.15, 0.6],
            [0.1, 0.55, 1.4, -0.1, 1.1, -0.3],
        ]),
        arr1(&[0.4, 0.15]),
    )?;
    let pre_act = BoundedTensor::new(
...
fn mean_only_fixture_4148() -> Result<(LayerNormLayer, LinearBounds, BoundedTensor, usize, usize)> {
    let layer = LayerNormLayer::new(arr1(&[0.5, -1.5, 2.0]), arr1(&[0.0, 0.25, -0.5]), 1e-5)?
        .with_mode(LayerNormMode::MeanOnly);
    let bounds = LinearBounds::new(
        arr2(&[
            [1.0, 0.25, -0.5, 0.1, 0.6, -0.8],
            [-0.4, 0.2, 0.9, 1.1, -0.3, 0.5],
        ]),
        arr1(&[-0.3, 0.2]),
        arr2(&[
            [1.2, 0.5, -0.25, 0.3, 0.8, -0.4],
            [-0.1, 0.45, 1.2, 1.4, 0.1, 0.8],
        ]),
        arr1(&[0.1, 0.45]),
    )?;
    let pre_act = BoundedTensor::new(
        ArrayD::from_shape_vec(IxDyn(&[2, 3]), vec![-0.4, 0.1, 0.9, -1.2, 0.3, 1.7]).unwrap(),
        ArrayD::from_shape_vec(IxDyn(&[2, 3]), vec![0.2, 0.8, 1.4, -0.4, 1.0, 2.2]).unwrap(),
    )?;
    Ok((layer, bounds, pre_act, 2, 3))
}

fn layernorm_multi_dim_case(
) -> impl Strategy<Value = (usize, usize, f32, Vec<f32>, Vec<f32>, Vec<f32>)> {
    (2usize..5, 2usize..5, 0.05f32..0.2).prop_flat_map(|(batch_size, norm_size, hw)| {
        (
            ...
            proptest::collection::vec(-0.5f32..0.5, norm_size),
        )
    })
}
...
#[ntest::timeout(10000)]
#[test]
fn test_crown_scalar_multi_dim_layernorm_matches_batched_4148() -> Result<()> {
    let (layer, bounds, pre_act, batch_size, norm_size) = standard_fixture_4148()?;
...
#[ntest::timeout(10000)]
#[test]
fn test_crown_scalar_multi_dim_layernorm_mean_only_matches_batched_4148() -> Result<()> {
    let (layer, bounds, pre_act, batch_size, norm_size) = mean_only_fixture_4148()?;
...
proptest! {
    ...
    #[ntest::timeout(60000)]
    #[test]
    fn proptest_crown_scalar_multi_dim_layernorm_contains_sampled_outputs_4148(
        (batch_size, norm_size, hw, centers, ny, beta) in layernorm_multi_dim_case(),
    ) {
        ...
        let result = layer
            ...
            .map_err(|e| {
                TestCaseError::fail(format!(
                    "multi-dim scalar LayerNorm CROWN failed: {e}"
                ))
            })?;
        ...
        for s in 0..12_u32 {
            ...
            for b in 0..batch_size {
                ...
                let y = layer.eval(&x).expect("LayerNorm eval should succeed");
                for i in 0..norm_size {
                    let idx = start + i;
                    prop_assert!(
                        concrete.lower()[[idx]] <= y[i] + 1e-4,
                        "multi-dim LayerNorm lower[{idx}] {} > {}",
                        concrete.lower()[[idx]],
                        y[i]
                    );
                    prop_assert!(
                        concrete.upper()[[idx]] >= y[i] - 1e-4,
                        "multi-dim LayerNorm upper[{idx}] {} < {}",
                        concrete.upper()[[idx]],
                        y[i]
                    );
                }
