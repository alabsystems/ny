...
/// Heavy soundness stress over the underflow regime: counts violations and total
/// samples across many seeds (no early panic on the count — it asserts 0 at the
/// end). `#[ignore]` so it does not bloat the default run; invoke deliberately:
/// `cargo nextest run -p ny-propagate -E 'test(soundness_underflow_stress_count)'
/// --run-ignored all -- --nocapture`.
#[test]
#[ignore = "heavy soundness stress count: run deliberately under --nocapture"]
fn soundness_underflow_stress_count() {
    let mut total = 0usize;
    let mut violations = 0usize;
    let masks: [(AttentionMask, Option<usize>); 3] = [
        (AttentionMask::Standard, None),
        (AttentionMask::Causal, None),
        (AttentionMask::Causal, Some(2)),
    ];
    for seed in 0..200u64 {
        for (mi, &(mask, window)) in masks.iter().enumerate() {
            let (t, v) = count_underflow_regime(seed + (mi as u64) * 100_000, mask, window);
            total += t;
            violations += v;
        }
    }
    eprintln!(
        "soundness_underflow_stress_count: samples_tested={total} soundness_violations={violations}"
    );
    assert_eq!(
        violations, 0,
        "underflow-regime enclosure violations must be 0"
    );
}

/// Like `check_underflow_regime` but RETURNS (samples, violations) instead of
/// panicking, for the stress counter. Identical box construction.
fn count_underflow_regime(seed: u64, mask: AttentionMask, window: Option<usize>) -> (usize, usize) {
    let mut rng = StdRng::seed_from_u64(seed);
    let (sq, sk, dk, dv) = (3usize, 3usize, 1usize, 2usize);
    let scale = 1.0f32;
    let dims = SliceDims {
        n_slices: 1,
        sq,
    ...
    for k in 0..sk {
        if k == under {
            let c = rng.random_range(-1000.0..-800.0);
            let r = rng.random_range(400.0..700.0);
            klo[k] = c - r;
            khi[k] = c + r;
        } else {
    ...
    let qbox = box_from_lo_hi(&[sq, dk], &qlo, &qhi);
    let kbox = box_from_lo_hi(&[sk, dk], &klo, &khi);
    let vbox = box_from_lo_hi(&[sk, dv], &vlo, &vhi);
    let n_y = sq * dv;
    let node_lb = identity_node_lb(n_y);
    let (bounds, blo, bhi) = layer
        .propagate_crown_ternary(&node_lb, &qbox, &kbox, &vbox)
    ...
    for _ in 0..n_samples {
        ...
        for o in 0..n_y {
            total += 1;
            let slack = 1e-3f32 + 1e-4 * y[o].abs();
            if !(y[o] >= cl[o] - slack && y[o] <= cu[o] + slack) {
                viol += 1;
            }
        }
    }
...
/// Peak-RSS / memory-boundedness measurement at table_transformer scale
/// (`num_heads=8, seq=64, head_dim=8`). `#[ignore]` — run deliberately under
/// `/usr/bin/time -l` to read peak RSS; the de-fused path OOMs (~20 GB) at this
/// scale, the fused ternary must stay far below. Reports the analytic surrogate
/// footprint. Run: `cargo nextest run -p ny-propagate -E
/// 'test(peak_rss_seq64)' --run-ignored all -- --nocapture`.
#[test]
#[ignore = "memory measurement: run deliberately under /usr/bin/time -l"]
fn peak_rss_seq64_table_transformer() {
    let heads = 8usize;
    let s = 64usize;
    let dk = 8usize;
    let dv = 8usize;
    ...
    // node_lb here uses a SMALL spec dim (a single linear readout over Y) to model
    // a realistic downstream spec, since num_outputs = spec rows, not seq². This
    // is the dominant output-matrix cost: 3 × (spec × n_in).
    let spec = 16usize;
    let mut rng = StdRng::seed_from_u64(11);
    let la: Vec<f32> = (0..spec * n_y)
        .map(|_| rng.random_range(-1.0..1.0))
        .collect();
    let lower_a = Array2::from_shape_vec((spec, n_y), la.clone()).unwrap();
    let upper_a = Array2::from_shape_vec((spec, n_y), la).unwrap();
    let node_lb =
        LinearBounds::new(lower_a, Array1::zeros(spec), upper_a, Array1::zeros(spec)).unwrap();
    ...
    let (qc, qr) = gen_cr(n_q, &mut rng, 1.0, 0.1);
    let (kc, kr) = gen_cr(n_q, &mut rng, 1.0, 0.1);
    let (vc, vr) = gen_cr(n_v, &mut rng, 1.0, 0.3);
    let qbox = box_from_center(&[heads, s, dk], &qc, &qr);
    let kbox = box_from_center(&[heads, s, dk], &kc, &kr);
    let vbox = box_from_center(&[heads, s, dv], &vc, &vr);
    let scale = 1.0 / (dk as f32).sqrt();
    let layer = SelfAttentionLayer::new(AttentionMask::Standard, Some(scale));

    let t0 = std::time::Instant::now();
    let (bounds, _blo, _bhi) = layer
        .propagate_crown_ternary(&node_lb, &qbox, &kbox, &vbox)
        .expect("seq=64 ternary CROWN should succeed");
    let dt = t0.elapsed();

    // Persistent output (the Nary result): 3 inputs × (lower+upper) × spec × n_in
    // × 4 bytes. The accumulator additionally holds f64 copies (×2). The TRANSIENT
    // per-slice Jacobian is only `(sq·dv × {sq·dk, sk·dk, sk·dv})` — one slice at a
    // time, reused — so the transient is O(seq²·d) per HEAD, never the full
    // `(heads·seq)²` softmax-Jacobian tensor that OOMs the de-fused path.
    let out_bytes: usize = bounds
        .iter()
        .flatten()
        .map(|lb| lb.lower_a().len() + lb.upper_a().len())
        .sum::<usize>()
        * 4;
    let per_slice_jac = (s * dv) * (s * dk + s * dk + s * dv) * 4; // jq+jk+jv, ONE slice
    eprintln!(
        "peak_rss_seq64: heads={heads} seq={s} dk={dk} dv={dv} spec={spec} | \
         Nary output ~{:.2} MB, per-slice transient Jacobian ~{:.2} MB (one slice live), \
         elapsed={:?}",
        out_bytes as f64 / 1e6,
        per_slice_jac as f64 / 1e6,
        dt,
    );
