...
//! Disjunctive GreaterEq PGD attack: finds counterexamples for `Y_j >= Y_target`.
//!
...
impl PgdAttacker<'_> {
    /// Attack disjunctions of the form `Y_j >= Y_target` for any `j`.
    pub fn attack_disjunctive_greater_eq(
        &self,
        network: &Network,
    ...
    #[cfg(test)]
    pub(super) fn attack_disjunctive_greater_eq_parallel_for_test(
        &self,
        network: &Network,
        input_bounds: &BoundedTensor,
        target_idx: usize,
...
mod tests {
    ...
    #[test]
    fn test_disjunctive_greater_eq_empty_comparison_indices_returns_err() {
        ...
        let result = attacker.attack_disjunctive_greater_eq(&network, &input_bounds, 0, &[]);
        assert!(
            result.is_err(),
