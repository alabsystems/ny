...
//! Quarantine audit for the retired resident-GPU Cut-CROWN C2 proof path.
//!
...
//! GPU-resident resnet backward (#w4-root-gpu route), then registers the old
//! public fold entries and proves every request is an exact no-op.
//!
//! Gate: `NY_CUT_FOLD_RESIDENT=1` plus any populated public registry must yield
//! zero applications and margins exactly equal to baseline.
//!
//! The retired C2b objective-signed experiment remains below as historical
//! research code, but the executable exits before GPU setup: the proof-path
//! reader and its capture branch are hard-quarantined.
//!
//! Invoking the former commands now prints the quarantine notice and exits
//! without GPU work:
//! `cargo run -p ny-onnx --release --example cut_fold_cifar100_resident -- c2`
//! or replace `c2` with `c2b`.
...
fn c2_cut_fold_resident_root_margin_experiment() {
    ...
    let device = ComputeDevice::new(Backend::Wgpu)
        .expect("a wgpu GPU adapter is required for the resident cut-fold experiment");
    ...
    // --- Gate (a2): even a populated λ=0 entry has no proof authority. ---
    ...
    assert_eq!(
        resident_cut_fold_applied_count(),
        0,
        "λ=0: resident proof path must not consume public fold state"
    );
    assert_eq!(
        base, lambda0,
    ...
    println!(
        "gate (a2) PASSED: λ=0 request quarantined (applied={}) and exactly equals baseline",
        resident_cut_fold_applied_count()
    );
    ...
    for &lambda in &[0.05f32, 0.1, 0.25, 0.5, 1.0] {
        ...
        assert_eq!(
            resident_cut_fold_applied_count(),
            0,
            "λ={lambda}: resident proof path must not consume public fold state"
        );
        assert_eq!(
            margins, base,
            "λ={lambda}: resident margins must remain exactly at baseline"
        );
    ...
    println!(
        "\nResident cut-fold quarantine PASSED: every populated registry request was an exact no-op"
    );
...
fn c2b_signed_cut_fold_resident_root_margin_experiment() {
    ...
    // --- A public registry request must neither fold nor expose proof-adjacent
    // frontier capture through the quarantined consumer. ---
    let _g_fold_on = ny_test_utils::env::ScopedEnvVar::set("NY_CUT_FOLD_RESIDENT", "1");
    ...
    assert_eq!(
        resident_cut_fold_applied_count(),
        0,
        "capture request must not acquire resident proof authority"
    );
    assert_eq!(
        base, cap_run,
        "gate FAILED: capture pass (split + empty fold) differs from baseline"
    );
    let Some(cap) = take_resident_cut_fold_capture() else {
        println!(
            "resident cut-fold/capture quarantine PASSED: request was an exact no-op; \
             objective-signed C2b measurement is retired"
        );
        clear_resident_cut_fold();
        return;
    };
    ...
    assert_eq!(
        resident_cut_fold_applied_count(),
        0,
        "λ=0: public resident fold must remain quarantined"
    );
    assert_eq!(
        base, lambda0,
    ...
    for (name, cuts) in variants {
        ...
        for &lambda in &grid {
            ...
            assert_eq!(
                resident_cut_fold_applied_count(),
                0,
                "λ={lambda}: public resident fold must remain quarantined"
            );
            assert_eq!(
                margins, base,
                "λ={lambda}: quarantined resident request must preserve baseline"
            );
            let m = min_of(&margins);
...
fn main() {
    match std::env::args().nth(1).as_deref() {
        Some("c2") => c2_cut_fold_resident_root_margin_experiment(),
        Some("c2b") => c2b_signed_cut_fold_resident_root_margin_experiment(),
        _ => panic!(
            "select exactly one experiment: \
             cargo run -p ny-onnx --release --example cut_fold_cifar100_resident -- c2|c2b"
        ),
    }
}
