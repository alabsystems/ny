...
//!
//! These external-checker tests are ignored by default and run explicitly by
//! the pinned Carcara CI gate. Invoking one without a reachable checker
//! (`$NY_CARCARA` or `carcara` on `PATH`) is a hard failure, never a skip.
...
#[test]
#[ignore = "requires pinned Carcara"]
